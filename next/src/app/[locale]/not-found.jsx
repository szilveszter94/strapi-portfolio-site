...
export async function generateMetadata(parent) {
  ...
  try {
    page = await fetchNotFoundPage(locale);
  } catch (error) {
...
export default async function NotFound() {
  const pathname = typeof window !== "undefined" ? window.location.pathname : "/";
  const locale = getLocaleFromPathname(pathname);
  const [page, global] = await Promise.allSettled([fetchNotFoundPage(locale), fetchLayout(locale)]);

  if (page.status === "rejected") {
    return (
      <div className="mx-auto max-w-5xl p-4">
        <div className="text-red-600 text-center">
          Error: We encountered an issue while loading the &quot;404 - Page not found&quot; page.
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <section className="triangle-path bg-neutral-100 py-16 relative after:content-[''] after:absolute after:top-0 after:right-0 after:w-1/4 after:h-full after:bg-neutral-200/50">
        <div className="relative mx-auto max-w-5xl px-4 text-center z-10">
          ...
          <BtnPrimary className="w-full sm:w-auto" label="Return to Home" url={`/${locale}/`} />
        </div>
      </section>
    </>
  );
