...
export default function Form({ locale }) {
  ...
  const tErrors = useTranslations("errors.zodValidation");
  const t = useTranslations("contactForm");
  const formSchema = createFormSchema(tErrors);
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-6 sm:gap-6">
        ...
        <div>
          ...
          <div id="email-error" className={`text-red-500 text-sm mt-1 ${errors.email ? "" : "sr-only"}`}>
        ...
        <div>
          <label className="relative block border border-neutral-300 bg-transparent rounded-lg">
            <textarea
              ...
              className="block rounded-lg peer w-full border-none bg-transparent px-4 py-2 text-gray-700 placeholder-transparent focus:border-transparent focus:outline-none text-xl"></textarea>
          ...
          <div id="message-error" className={`text-red-500 text-sm mt-1 ${errors.message ? "" : "sr-only"}`}>
        ...
        <div>
          ...
          <div id="consent-error" className={`text-red-500 text-sm mt-1 ${errors.consent ? "" : "sr-only"}`}>
      ...
      {hasServerError && <div className="text-red-500 text-sm mt-1">{t("error")}</div>}
      {isSubmitted && <div className="text-green-500 text-sm mt-1">{t("success")}</div>}
    </>
  );
}
